...
class ESPHelper {
  ...
  public:
    ...
    // remember netInfo *netList[] is really netinfo**
    ESPHelper(netInfo *netList[], uint8_t netCount, uint8_t startIndex = 0);

    ESPHelper(const char *ssid, const char *pass, const char *mqttIP);

    ESPHelper(const char *ssid, const char *pass);

    ESPHelper(const char *ssid,
              const char *pass,
              const char *mqttIP,
              const char *mqttUser,
              const char *mqttPass,
    ...
    // manually disconnect and reconnecting to network/mqtt using current values
    // (generally called after setting new network values)
    void updateNetwork();
    ...
    void setWill(const char *willTopic,
                 const char *willMessage,
                 const int willQoS,
                 const int willRetain);
